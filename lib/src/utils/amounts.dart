...
class EtherAmount {
  ...
  static EtherAmount fromUnitAndValue(EtherUnit unit, dynamic amount) {
    ...
    if (amount is BigInt) {
      ...
      parsedAmount = BigInt.from(int.parse(amount));
    } else {
      throw ArgumentError('Invalid type, must be BigInt, string or int');
    }

    return EtherAmount.inWei(parsedAmount * factors[unit]);
  }
  ...
  num getValueInUnit(EtherUnit unit) {
    final value = _value ~/ factors[unit];
    final remainder = _value.remainder(factors[unit]);
