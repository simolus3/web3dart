...
class Web3Client {
  ...
  /// an [EthPrivateKey].
  Future<Credentials> credentialsFromPrivateKey(String privateKey) async {
  ...
  Future<int> getNetworkId() {
    return _makeRPCCall<String>('net_version')
        .then(int.parse as FutureOr<int> Function(String?));
  }
  ...
  Future<int> getPeerCount() async {
    final hex =
        await (_makeRPCCall<String>('net_peerCount') as FutureOr<String>);
  ...
  Future<int> getEtherProtocolVersion() async {
    final hex =
        await (_makeRPCCall<String>('eth_protocolVersion') as FutureOr<String>);
    return hexToInt(hex).toInt();
  }
  ...
  Future<EthereumAddress> coinbaseAddress() async {
    final hex =
        await (_makeRPCCall<String>('eth_coinbase') as FutureOr<String>);
  ...
  Future<EtherAmount> getGasPrice() async {
    final data =
        await (_makeRPCCall<String>('eth_gasPrice') as FutureOr<String>);
  ...
  Future<TransactionReceipt?> getTransactionReceipt(String hash) async {
    final call = await _makeRPCCall<Map<String, dynamic>>(
        'eth_getTransactionReceipt', [hash]);
    return (call != null) ? TransactionReceipt.fromMap(call) : null;
  }
  ...
  Future<Uint8List> signTransaction(Credentials cred, Transaction transaction,
    ...
    return internalSign(signingInput);
  }
