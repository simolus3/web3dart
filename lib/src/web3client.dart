...
class Web3Client {
  ...
  Web3Client(String connectionUrl, Client _httpClient) {
    _jsonRpc = JsonRPC(connectionUrl, _httpClient);
  }
  ...
  Future<EtherAmount> getBalance(EthereumAddress address, {BlockNum atBlock}) {
    ...
    return _makeRPCCall<String>('eth_getBalance', [address.hex, blockParam])
      ...
      return EtherAmount.fromUnitAndValue(
          EtherUnit.wei, numbers.hexToInt(data));
    });
  }
  ...
  Future<List<int>> getStorage(EthereumAddress address, BigInt position,
    ...
    return _makeRPCCall<String>('eth_getStorageAt', [
      address.hex,
      numbers.toHex(position, pad: true, include0x: true),
      blockParam
  ...
  Future<List<int>> sendRawTransaction(
      Credentials cred, RawTransaction transaction,
      {int chainId}) {
    final data =
...
class BlockNum {
  ...
  String toBlockParam() {
    if (useAbsolute) return numbers.toHex(blockNum, pad: true, include0x: true);
