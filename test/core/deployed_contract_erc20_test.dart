...
class ERC20Contract extends DeployedContract {
  ...
  Future<String> name() => invoker(web3, 'name').call<String>();

  @StateMutability.view
  Future<BigInt> decimals() => invoker(web3, 'decimals').call();

  @StateMutability.view
  Future<BigInt> totalSupply() => invoker(web3, 'totalSupply').call();

  @StateMutability.view
  Future<String> symbol() => invoker(web3, 'symbol').call();

  @StateMutability.view
  Future<BigInt> blanaceOf(EthereumAddress owner) =>
      invoker(web3, 'balanceOf').parameters(owner).call<BigInt>();

  @StateMutability.view
  Future<BigInt> allowance(EthereumAddress owner, EthereumAddress spender) =>
      invoker(web3, 'allowance').parameters([owner, spender]).call(from: owner);

  @StateMutability.nonPayable
  ContractInvocation transfer(EthereumAddress recipient, BigInt amount) =>
      invoker(web3, 'transfer').parameters([recipient, amount]);

  @StateMutability.nonPayable
  ContractInvocation approve;

  @StateMutability.nonPayable
  ContractInvocation transferFrom;

  ERC20Contract(this.web3, EthereumAddress contractAddress)
      : super(
...
//
  ...
  test('test ERC20 infomations', () async {
    ...
    final erc20 = ERC20Contract(web3,
        EthereumAddress.fromHex('0xf4A21353da36389a924ab144DcC694BEf1B1d0F2'));

    /// test address
    final account = EthPrivateKey.fromHex(
        '0x9a28e3c832827b79e856b1868289ad99dbdab3f8cf4e7e9d1932069afe9e0fd3');
    final fromAddress = await account.extractAddress();
    final toAddress =
        EthereumAddress.fromHex('0xB661639fb5A9C5F18E37aB567FCd65B8890D1E3e');
    ...
    print(
        'TotalSupply: ${EtherAmount.fromWei(await erc20.totalSupply()).getValueInUnit(EtherUnit.ether)}');

    final invoker =
        erc20.transfer(toAddress, EtherAmount.fromEther(1).getInWei);

    print(
        'Begin transfer: 0xf5DdA759346F8d185e04D14D4457729394D3ce00 -> 0xB661639fb5A9C5F18E37aB567FCd65B8890D1E3e : 1.00 ${await erc20.name()}');
    ...
    print('Balnace:');
    print(
        '0xf5DdA759346F8d185e04D14D4457729394D3ce00: ${EtherAmount.fromWei(await erc20.blanaceOf(fromAddress)).getValueInUnit(EtherUnit.ether)}');
    print(
        '0xB661639fb5A9C5F18E37aB567FCd65B8890D1E3e: ${EtherAmount.fromWei(await erc20.blanaceOf(toAddress)).getValueInUnit(EtherUnit.ether)}');
